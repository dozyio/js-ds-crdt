...
describe('CRDTSet', () => {
  ...
  beforeEach(() => {
    ...
    log = logger('test');
    crdtSet = new CRDTSet(store, namespace, log, putHookMock, deleteHookMock);
  });
  ...
  it('should correctly handle priority when adding elements', async () => {
    ...
    await crdtSet.putElems([{ key, value: lowValue, id: 'id1' }], 'id1', BigInt(1));

    // Now, add an element with a higher priority
    await crdtSet.putElems([{ key, value: highValue, id: 'id2' }], 'id2', BigInt(2));
  ...
  it('should resolve conflicts by lexicographical order of IDs when priorities are equal', async () => {
    ...
    await crdtSet.putElems([{ key, value: value1, id: 'idA' }], 'idA', BigInt(1));
    await crdtSet.putElems([{ key, value: value2, id: 'idB' }], 'idB', BigInt(1));
  ...
  it('should handle merging an empty delta', async () => {
    ...
    await crdtSet.putElems([{ key, value: value, id: 'id1' }], 'id1', BigInt(1));
  ...
  it('should handle concurrent additions correctly', async () => {
    ...
    const put1 = crdtSet.putElems([{ key, value: value1, id: 'id1' }], 'id1', BigInt(1));
    const put2 = crdtSet.putElems([{ key, value: value2, id: 'id2' }], 'id2', BigInt(2));
  ...
  it('should correctly sync the datastore with existing keys', async () => {
    ...
    await crdtSet.putElems([{ key: key1, value: value1, id: 'id1' }], 'id1', BigInt(1));
