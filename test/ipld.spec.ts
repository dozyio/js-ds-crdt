...
describe('CrdtNodeGetter', () => {
  ...
  beforeEach(() => {
    ...
    crdtNodeGetter = new CrdtNodeGetter(blockstore);
  });

  it('should correctly get delta for a given CID', async () => {
    const cid = CID.parse('bafybeihdwdcefgh4dqkjv67uzcmw7ojee6xedzdetojuzjevtenxquvyku');

    const mockDelta: delta.Delta = {
      priority: BigInt(10),
      elements: [
        { key: 'key1', id: 'id1', value: new Uint8Array([1, 2, 3]) }
      ],
      tombstones: [
    ...
    const block = await Block.encode({ value: codec.createNode(encodedDelta, []), codec, hasher });
  ...
  it('should correctly handle missing data in node', async () => {
    const cid = CID.parse('bafybeihdwdcefgh4dqkjv67uzcmw7ojee6xedzdetojuzjevtenxquvyku');
    const block = await Block.encode({ value: codec.createNode(new Uint8Array(), []), codec, hasher });

    await blockstore.put(cid, block.bytes);

    await expect(crdtNodeGetter.getDelta(cid)).rejects.toThrow('Node has no data');
  });
  ...
  it('should yield deltas for multiple CIDs', async () => {
    ...
    const block1 = await Block.encode({ value: codec.createNode(encodedDelta1, []), codec, hasher });
    const block2 = await Block.encode({ value: codec.createNode(encodedDelta2, []), codec, hasher });
