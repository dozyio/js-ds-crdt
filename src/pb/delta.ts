            ...
            switch (tag >>> 3) {
              case 1: {
                if (opts.limits?.elements != null && obj.elements.length === opts.limits.elements) {
                  throw new MaxLengthError('Decode error - map field "elements" had too many elements')
                }

                obj.elements.push(delta.Element.codec().decode(reader, reader.uint32(), {
              ...
              case 2: {
                if (opts.limits?.tombstones != null && obj.tombstones.length === opts.limits.tombstones) {
                  throw new MaxLengthError('Decode error - map field "tombstones" had too many elements')
                }

                obj.tombstones.push(delta.Element.codec().decode(reader, reader.uint32(), {
