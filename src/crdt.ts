...
import { Key, type Datastore as DSDatastore, type Batch as DSBatch } from 'interface-datastore';
...
// Common errors.
const ErrNoMoreBroadcast = new Error("receiving blocks aborted since no new blocks will be broadcasted");
...
export class Datastore {
  ...
  constructor(
    ...
    this.set = new CRDTSet(store, namespace.child(new Key(setNs)), this.logger, this.options.putHook, this.options.deleteHook);
    this.heads = new Heads(store, namespace.child(new Key(headsNs)), this.logger);
  ...
  private async dagWorker(): Promise<void> {
    while (true) {
      ...
      try {
        const children = await this.processNode(job.root, job.rootPrio, job.delta, job.node);
        this.enqueueSendJobs(new DagJob(job.session, job.nodeGetter, job.root, job.rootPrio, job.delta, job.node, children));
      } catch (error) {
  ...
  private async repair(): Promise<void> {
    ...
    while (!this.ctx.signal.aborted) {
      await new Promise(resolve => setTimeout(resolve, this.options.repairInterval));
      clearTimeout(timer);
  ...
  private async logStats(): Promise<void> {
    ...
    while (!this.ctx.signal.aborted) {
      ...
      const heads = await this.heads.list();
      this.logger.trace(`Number of heads: ${heads.heads.length}. Max height: ${heads.maxHeight}. Queued jobs: ${this.jobQueue.length}. Dirty: ${await this.IsDirty()}`);
    }
  }
  ...
  private async decodeBroadcast(data: Uint8Array): Promise<CID[]> {
    ...
    if (bcastData && bcastData.Heads) {
      return Promise.all(bcastData.Heads.map((protoHead) => CID.decode(protoHead.Cid)));
    }
  ...
  private async sendNewJobs(session: Mutex, ng: CrdtNodeGetter, root: CID, rootPrio: bigint, children: CID[]): Promise<void> {
    ...
    for await (const deltaOpt of ng.getDeltas(children)) {
      ...
      const value = createNode(deltaOpt.node.Data || new Uint8Array(), deltaOpt.node.Links);
      const block = await Block.encode({ value, codec, hasher })

      goodDeltas.set(block.cid, true);

      const job = new DagJob(session, ng, root, rootPrio, deltaOpt.delta, deltaOpt.node, []);
      this.sendJobs.push(job);
    }
  ...
  private processedBlockKey(c: CID): Key {
    return this.namespace.child(new Key(processedBlocksNs)).child(new Key(c.toString()));
  }
  ...
  private async putBlock(heads: CID[], height: bigint, delta: dpb.delta.Delta): Promise<dagPb.PBNode> {
    delta.priority = height;
  ...
  public async processNode(root: CID, rootPrio: bigint, delta: dpb.delta.Delta, node: dagPb.PBNode): Promise<CID[]> {
    ...
    try {
      ...
      this.logger.error(`Merged delta from node ${current} (priority: ${delta.priority})`);
  ...
  private async printDAGRec(from: CID, depth: number, getter: CrdtNodeGetter, set: Set<string>): Promise<void> {
  ...
  private mergeDeltas(d1: dpb.delta.Delta, d2: dpb.delta.Delta): dpb.delta.Delta {
    return {
  ...
  private async updateDeltaWithRemove(key: string, newDelta: dpb.delta.Delta): Promise<number> {
    return await this.curDeltaMutex.runExclusive(() => {
      if (this.curDelta) {
