...
export class CRDTSet {
  ...
  constructor(
    ...
    this.tombstonesBloom = new BloomFilter(TombstonesBloomFilterSize, TombstonesBloomFilterHashes);
  ...
  public async elements(q: Query): Promise<Pair[]> {
    ...
    const setQueryPrefix = keyNamespacePrefix.child(srcQueryPrefixKey).toString();
  ...
  private valueKey(key: string): Key {
    return this.keyPrefix(keysNs).child(new Key(key)).child(new Key(valueSuffix));
  }

  private priorityKey(key: string): Key {
    return this.keyPrefix(keysNs).child(new Key(key)).child(new Key(prioritySuffix));
  }
  ...
  private async setValue(
    ...
    if (prio > curPrio || (prio === curPrio && Buffer.compare(value, await this.store.get(this.valueKey(key))) > 0)) {
  ...
  private async getPriority(key: string): Promise<bigint> {
    ...
    try {
      ...
      if (error.code === 'ERR_NOT_FOUND' || error.message.includes('Not Found')) {
  ...
  private async setPriority(writeStore: Datastore, key: string, prio: bigint): Promise<void> {
    const prioK = this.priorityKey(key);
  ...
  public async putElems(elems: pb.delta.Element[], id: string, prio: bigint): Promise<void> {
